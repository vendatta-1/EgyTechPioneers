...
public class StudentEvaluationController(IStudentEvaluation service) : ControllerBase
{
    [HttpGet("{id:guid}")]
    ...
    [HttpPost]
    public async Task<IResult> Create([FromBody] StudentEvaluationDto dto, CancellationToken ct)
    {
        var result = await service.CreateAsync(dto, ct);
        return result.Match(
            data => Results.Created($"/api/StudentEvaluation/{data.Id}", data),
            ApiResults.Problem
        );
    }
