...
public class StudentGroupController(IStudentGroup service) : ControllerBase
{
    [HttpGet("{id:guid}")]
    ...
    [HttpPost]
    public async Task<IResult> Create([FromBody] StudentGroupDto dto, CancellationToken ct)
    {
        var result = await service.CreateAsync(dto, ct);
        return result.Match(
            data => Results.Created($"/api/StudentGroup/{data.Id}", data),
            ApiResults.Problem
        );
    }
