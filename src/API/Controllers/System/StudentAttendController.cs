...
public class StudentAttendController(IStudentAttend service) : ControllerBase
{
    [HttpGet("{id:guid}")]
    ...
    [HttpPost]
    public async Task<IResult> Create([FromBody] StudentAttendDto dto, CancellationToken ct)
    {
        var result = await service.CreateAsync(dto, ct);
        return result.Match(
            data => Results.Created($"/api/StudentAttend/{data.Id}", data),
            ApiResults.Problem
        );
    }
